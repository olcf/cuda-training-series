...
int main(){
  ...
  gaussian_pdf<<<(ds+255)/256, 256>>>(d_x, d_y, 0.0, 1.0, ds); // warmp-up
  ...
  for (int i = 0; i < ds; i++) if (h_y[i] != h_y1[i]) {std::cout << "mismatch at " << i << " was: " << h_y[i] << " should be: " << h_y1[i] << std::endl; return -1;}
